...
def json_to_df(data, save=False):
    ...
    for parish_data in data.get("parishes", []):
        parish_name = parish_data.get("parish", "")
        prev_townland = None
        for entry in parish_data.get("entries", []):
            townland_raw = entry.get("townland", "")
            if townland_raw != "":
                prev_townland = townland_raw
            townland_filled = townland_raw if townland_raw != "" else prev_townland
            df = df._append(
                {
                    "parish": parish_name,
                    "mr": entry.get("mr", ""),
                    "townland_raw": townland_raw,
                    "townland": townland_filled,
                    "os": entry.get("os", ""),
                    ...
                    "n_shared": entry.get("n_shared", 1),
                    "is_total": entry.get("is_total", 0),
                    "is_exemption": entry.get("is_exemption", 0),
                    "is_continued": entry.get("is_continued", 0),
                },
                ignore_index=True,
            )
    return df
...
def to_total_pence(value):
    ...
    pounds = _part(parts[0])
    shillings = _part(parts[1])
    pence = _part(parts[2])
...
def check_if_correct(total_land_val, total_total_val, sum_land_val, sum_total_val):
    ...
    return off_by_factor_of_two(total_land_val, sum_land_val) and off_by_factor_of_two(
        total_total_val, sum_total_val
    )
...
def find_best_townland_match(expected_townland, available_townlands, threshold=0.65):
    ...
    for townland in available_townlands:
        score = SequenceMatcher(
            None, clean_townland(expected_townland), clean_townland(townland)
        ).ratio()
        if score > best_score:
            best_score = score
            best_match = townland
...
def get_total_values(df, townland):
    ...
    if len(df_townland) == 0:
        ...
        if matched_townland:
            ...
            raise ValueError(
                f"Townland '{townland}' not found in extracted data (best match score: {score*100:.1f}%). "
                f"Available townlands: {available_townlands}"
            )
    ...
    for _, row in df_townland[df_townland["is_total"] == 0].iterrows():
    ...
    return {
        "sum_land_val": sum_land_val,
        "total_land_val": total_land_val,
        "sum_total_val": sum_total_val,
        "total_total_val": total_total_val,
